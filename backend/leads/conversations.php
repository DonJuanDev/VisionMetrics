...
foreach ($conversations as $conv) {
    if (strpos($conv['utm_source'] ?? '', 'meta') !== false || strpos($conv['utm_source'] ?? '', 'facebook') !== false || !empty($conv['fbclid'])) {
        $metaAds++;
    }
}

// Get Google Ads count
$googleAds = 0;
foreach ($conversations as $conv) {
    if (strpos($conv['utm_source'] ?? '', 'google') !== false || !empty($conv['gclid'])) {
        $googleAds++;
    }
}
?>
